...
Liste des gènes cités dans l'article :
...
Liste des gènes d'expression différentielle (padj inferieure à ALPHA) :
...
Est-ce qu'on trouve des gènes cités dans l'article parmi les gènes d'expression différencielle ?

```{r genes article et diff}

any(a_genes %in% d_genes)
```

### 3. Plots et Analyses pour les gènes de l'article

#### 3a. Plotcounts pour les gènes mentionnées dans l'article
...
#### 3b. PlotMA pour les gènes mentionnés dans l'article
...
#### 3c. Ggplot pour les gènes mentionnés dans l'article

```{r ARTICLE ggplot ExpDiff}
ggplot(as.data.frame(res_Article), aes(x = baseMean, y = log2FoldChange)) + geom_point() + 
  ggtitle("Expression différentielle des gènes de l'article :") +
  ylab("log2 Fold Change") + xlab("BaseMean") + geom_text(label=OrdArticle$a_gene_name)
```

#### 3d. Heatmap pour les gènes mentionnés dans l'article
...
#### 3e. VolcanoPlot pour les gènes mentionnés dans l'article
...
text(res_Article$log2FoldChange,
     ...
     lab=OrdArticle$a_gene_name, cex=0.4)
```

Conclusion : on ne trouve pas les mêmes résultats que dans l'article quels gèenes on trouve nous ? et quels sont-ils ?

### 4. Plots et Analyses pour les gènes d'expression différentielle

#### 4a. Plotcounts pour les gènes d'expression différentielle
...
#### 4c. PlotMA pour les gènes d'expression différentielle
...
#### 4d. Ggplot pour les gènes d'expression différentielle
...
#### 4e. Heatmap pour les gènes d'expression différentielle

```{r Check no NAs, eval = FALSE, include = FALSE}
...
```{r Drop NAs, eval = FALSE, include = FALSE}
...
#### 4f. VolcanoPlot pour les gènes d'expression différentielle
